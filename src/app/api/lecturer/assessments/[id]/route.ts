...
import { purgeAttemptData } from "@/lib/assessment-reset";
import type {
...
async function updateQuestionWithRubrics(
	tx: AssessmentTx,
	q: QuestionPayload,
	sectionId: number,
	groupId?: number,
...
async function updateExistingAssessmentContent(
	tx: AssessmentTx,
	assessmentId: number,
	...
	if (
		...
		throw new StructuralEditError(
			"Closed assessments with submissions cannot add or remove sections",
		);
	}
	...
	for (const s of sections) {
		...
		if (
			!sameIds(
				existingSection.questions.map((q) => q.id),
				s.questions.map((q) => q.id),
			)
		) {
			throw new StructuralEditError(
				"Closed assessments with submissions cannot add or remove standalone questions",
			);
		}
		if (
			!sameIds(
				existingSection.groups.map((g) => g.id),
				(s.groups ?? []).map((g) => g.id),
			)
		) {
			throw new StructuralEditError(
				"Closed assessments with submissions cannot add or remove groups",
			);
		}
		...
		for (const g of s.groups ?? []) {
			...
			if (
				!sameIds(
					existingGroup.questions.map((q) => q.id),
					g.questions.map((q) => q.id),
				)
			) {
				throw new StructuralEditError(
					"Closed assessments with submissions cannot add or remove grouped questions",
				);
			}
...
export async function PUT(
	...
	if (existing.status !== "DRAFT" && existing.status !== "CLOSED") {
		return NextResponse.json(
			{ error: "Only DRAFT or CLOSED assessments can be edited" },
			{ status: 400 },
		);
	}
	...
	if (body.requireTrustedNetwork) {
		const network = await prisma.trustedNetwork.findFirst({
			where: { id: body.trustedNetworkId ?? -1, enabled: true },
			select: { id: true },
		});
		if (!network)
			return NextResponse.json(
				{ error: "Select an active approved network" },
				{ status: 400 },
			);
	}
	const passwordErr = validatePasswordProtection(
	...
	const preserveExistingContent =
		existing.status === "CLOSED" && attemptCount > 0;
	...
	try {
		const updated = await prisma.$transaction(async (tx) => {
			const a = await tx.assessment.update({
				...
				data: {
					...
					trustedNetworkId: body.requireTrustedNetwork
						? body.trustedNetworkId
						: null,
			...
			if (preserveExistingContent) {
				await updateExistingAssessmentContent(
					tx,
					assessmentId,
					body.sections ?? [],
				);
		...
		console.error(
			"[PUT /api/lecturer/assessments/[id]] Failed to update assessment",
			{
				assessmentId,
				lecturerId,
...
export async function DELETE(
	...
	try {
		// A plain `assessment.delete()` fails: attempts (and their proctor records +
		// grader feedback) reference the assessment without an ON DELETE cascade.
		// Purge every attempt and its dependents first, then the assessment itself
		// cascade-deletes its sections, questions, groups, rubrics and classes.
		await prisma.$transaction(async (tx) => {
			const attempts = await tx.assessmentAttempt.findMany({
				where: { assessmentId },
				select: { id: true },
			});
			await purgeAttemptData(
				tx,
				attempts.map((a) => a.id),
			);
		...
		console.error(
			"[DELETE /api/lecturer/assessments/[id]] Failed to delete assessment",
			{
				assessmentId,
				lecturerId,
				error: err instanceof Error ? err.message : String(err),
