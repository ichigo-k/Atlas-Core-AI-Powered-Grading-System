...
function SortHeader({
	...
	return (
		...
		>
			...
			<ArrowUpDown
				size={11}
				className={column.getIsSorted() ? "text-[#002388]" : "text-slate-300"}
			/>
		</button>
	);
}
	...
	return [
		{
			...
			cell: ({ row }) => (
				<div>
					...
					<p className="text-[11px] text-muted-foreground">
						{row.original.indexNumber ? `${row.original.indexNumber} / ` : ""}
						{row.original.email}
					</p>
				</div>
			),
			filterFn: (row, _id, value) => {
				...
				return (
					...
					(row.original.indexNumber ?? "").toLowerCase().includes(q)
				);
			},
		},
		{
		...
		},
		{
			...
			header: ({ column }) => (
				<SortHeader label="Assessments" column={column} />
			),
			cell: ({ row }) => {
				...
				return (
					<div className="flex items-center gap-2">
						...
						<span className="text-[12px] font-semibold text-[#1e293b] tabular-nums">
							{n}
						</span>
					</div>
				);
...
export default function GradebookClient() {
	...
	useEffect(() => {
		...
		if (courseFilter !== "all")
			filters.push({ id: "courseIds", value: courseFilter });
		if (classFilter !== "all")
			filters.push({ id: "classId", value: classFilter });
		if (levelFilter !== "all")
			filters.push({ id: "classLevel", value: levelFilter });
	...
	return (
		<div className="space-y-5">
			...
			<p className="text-[11px] text-muted-foreground">
				Showing{" "}
				<span className="text-[#1e293b] font-semibold">{filteredCount}</span> of{" "}
				<span className="text-[#1e293b] font-semibold">{totalStudents}</span>{" "}
				students
				...
				<div className="flex flex-col items-center justify-center rounded-sm border border-dashed border-border bg-white px-6 py-20 text-center">
					...
					{hasFilters && (
						<button
							type="button"
							onClick={clearFilters}
							className="mt-4 rounded-sm border border-border px-4 py-2 text-[12px] font-semibold text-[#323130] hover:bg-slate-50 transition-colors"
						>
							Clear filters
						</button>
					)}
					...
					<Table>
						<TableHeader>
							{table.getHeaderGroups().map((hg) => (
								<TableRow
									key={hg.id}
									className="bg-slate-50 hover:bg-slate-50 border-b border-border"
								>
