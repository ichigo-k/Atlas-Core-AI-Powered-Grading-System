		...
		<div className="px-4 py-5 md:px-6 lg:px-8 max-w-[1280px] pb-16 space-y-5 animate-pulse">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				{[...Array(3)].map((_, i) => (
					<div
						key={i}
						className={`rounded-sm border border-border bg-white p-5 space-y-3 ${i === 2 ? "md:col-span-2" : ""}`}
					>
						<Skeleton className="h-3 w-24" />
						{[...Array(4)].map((_, j) => (
...
async function AssessmentData({
	...
	const assessment: AssessmentWithDetails = {
		...
		classes: raw.classes.map(
			(ac: { id: number; classId: number; class: { name: string } }) => ({
				id: ac.id,
				classId: ac.classId,
	...
	if (resolvedStatus !== "DRAFT") {
		...
		if (aWithStudents) {
			...
			const hasSubjectiveSections = aWithStudents.sections.some(
				(s: any) => s.type === "SUBJECTIVE",
			);
			const scoreVisible =
				raw.gradingStatus === "GRADED" || !hasSubjectiveSections;
			...
			const plagiarismByAttemptId = new Map<number, boolean>(
				gradingResults.map((gr: any) => [gr.attemptId, gr.plagiarismFlagged]),
			);

			const submissionMap = new Map<
				number,
				AssessmentResultsData["submissions"][number]
			>();
			for (const attempt of attempts) {
				...
				const isHigher =
					...
					(attempt.score !== null &&
						(existing.score === null ||
							attempt.score > (existing.score ?? -Infinity)));
				if (isHigher) {
			...
			resultsData = {
				...
				courseTitle: raw.course.title,
				totalMarks: raw.totalMarks,
				totalQuestions: aWithStudents.sections.reduce(
