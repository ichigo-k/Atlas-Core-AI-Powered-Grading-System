...
function MetaRow({
	...
	return (
		<div className="flex items-center gap-3 py-2.5 border-b border-[#f1f5f9] last:border-0">
			<div className="w-4 shrink-0 text-muted-foreground flex items-center justify-center">
				{icon}
			</div>
...
function StatTile({
	...
	return (
		<div className="flex flex-col items-center justify-center gap-1 px-6 py-5 border-r border-[#f1f5f9] last:border-r-0 group">
			...
			<span className="text-[26px] font-bold text-[#1e293b] leading-none tabular-nums">
				{value}
			</span>
...
const statusBadge: Record<string, { cls: string; dot: string }> = {
	...
	PUBLISHED: {
		cls: "bg-[#dcfce7] text-[#166534] border-[#bbf7d0]",
		dot: "bg-[#22c55e]",
	},
	CLOSED: {
...
function EditSettingsSheet({
	...
	const [endsAt, setEndsAt] = useState(
		toDatetimeLocal(new Date(assessment.endsAt)),
	);
	...
	function handleSave() {
		startTransition(async () => {
			...
			const res = await fetch(
				`/api/lecturer/assessments/${assessment.id}/settings`,
				{
					method: "PATCH",
		...
		<Sheet open={open} onOpenChange={(v) => !v && onClose()}>
			...
			>
				<SheetHeader className="px-6 py-4 border-b border-border shrink-0">
					<SheetTitle className="text-[14px] font-semibold text-[#1e293b]">
						Edit Settings
					</SheetTitle>
...
function sectionUnitCount(
	s: AssessmentWithDetails["sections"][number],
): number {
...
function sectionUnitMarks(
	...
	const list =
		required != null && required < unitMarks.length
...
export default function AssessmentView({
	...
	useEffect(() => {
		...
		async function poll() {
			...
			try {
				const res = await fetch(
					`/api/lecturer/assessments/${assessment.id}/status`,
				);
	...
	function handleReopen() {
		startReopening(async () => {
			...
			if (!res.ok) {
				const d = await res.json();
				toast.error(d.error ?? "Failed to re-open");
				return;
			}
	...
	async function handleDelete() {
		setIsDeleting(true);
		try {
			const res = await fetch(`/api/lecturer/assessments/${assessment.id}`, {
				method: "DELETE",
			});
			if (!res.ok) throw new Error();
	...
	function handleStartGrading() {
		startGrading(async () => {
			const res = await fetch(
				`/api/lecturer/assessments/${assessment.id}/start-grading`,
				{ method: "POST" },
			);
			if (!res.ok) {
	...
	function handleReleaseResults() {
		startReleasing(async () => {
			const res = await fetch(
				`/api/lecturer/assessments/${assessment.id}/release-results`,
				{ method: "POST" },
			);
			if (!res.ok) {
				const body = await res.json().catch(() => ({}));
				toast.error(
					body.error === "Cannot release results before grading is complete"
	...
	function handleUnreleaseResults() {
		startUnreleasing(async () => {
			const res = await fetch(
				`/api/lecturer/assessments/${assessment.id}/unrelease-results`,
				{ method: "POST" },
			);
			if (!res.ok) {
	...
	async function handleRegrade(attemptId: number) {
		...
		try {
			const res = await fetch(
				`/api/lecturer/assessments/${assessment.id}/attempts/${attemptId}/regrade`,
				{ method: "POST" },
			);
			if (res.status === 429) {
				...
				toast.error(
					`Rate limited. Try again in ${mins} minute${mins !== 1 ? "s" : ""}.`,
				);
				return;
			}
			if (!res.ok) {
				toast.error("Re-grading failed.");
				return;
			}
	...
	async function handleExport() {
		...
		try {
			...
			a.href = objectUrl;
			a.download = `marks-${assessment.id}.xlsx`;
	...
	const submissionByStudent = useMemo(
		() =>
			new Map(
				(resultsData?.submissions ?? []).map((s: any) => [s.studentId, s]),
			),
		[resultsData],
	);
	...
	const tableRows = useMemo(() => {
		...
		const filtered = resultsData.enrolledStudents.filter((s: any) => {
			...
			const matchesFilter =
				resultsFilter === "ALL" ||
				(resultsFilter === "NOT_SUBMITTED" && !sub) ||
				(resultsFilter === "SUBMITTED" && !!sub) ||
				(resultsFilter === "GRADED" && sub?.status === "GRADED") ||
				(resultsFilter === "FLAGGED" && sub?.plagiarismFlagged === true);
		...
		return [...filtered].sort((a, b) => {
			...
			if (sortKey === "name") cmp = a.name.localeCompare(b.name);
				...
				cmp =
					(ord[subA?.status as keyof typeof ord] ?? 2) -
					(ord[subB?.status as keyof typeof ord] ?? 2);
			} else if (sortKey === "score")
				cmp = (subA?.score ?? -1) - (subB?.score ?? -1);
			else if (sortKey === "submittedAt") {
				cmp =
					(subA?.submittedAt ? new Date(subA.submittedAt).getTime() : 0) -
	...
	const totalQuestions = assessment.sections.reduce(
		(acc, s) => acc + sectionUnitCount(s),
		0,
	);
	const status = statusBadge[assessment.status] ?? statusBadge.DRAFT;
	const submittedCount = resultsData?.submissions.length ?? 0;
	const gradedCount = (resultsData?.submissions ?? []).filter(
		(s: any) => s.status === "GRADED",
	).length;
	const notSubmittedCount =
		(resultsData?.enrolledStudents.length ?? 0) - submittedCount;
	const submissionRate =
		(resultsData?.enrolledStudents.length ?? 0) > 0
			? Math.round(
					(submittedCount / (resultsData?.enrolledStudents.length ?? 1)) * 100,
				)
			: 0;

	// Chart data
	const scoredSubmissions = (resultsData?.submissions ?? []).filter(
		(s: any) => s.score != null,
	);
	const resultFilterOptions: Array<{
	...
	}> = [
		{
			key: "ALL",
			label: "All",
			count: resultsData?.enrolledStudents.length ?? 0,
		},
		...
		{ key: "GRADED", label: "Graded", count: gradedCount },
		{
			key: "FLAGGED",
			label: "Flagged",
			count: (resultsData?.submissions ?? []).filter(
				(s: any) => s.plagiarismFlagged === true,
			).length,
		},
	];
	...
	const doughnutOptions = {
		...
		plugins: {
			...
			tooltip: {
				callbacks: {
					label: (ctx: { parsed: number }) => ` ${ctx.parsed} students`,
				},
			},
		},
	};
	...
	function ActionBar() {
		return (
			<div className="flex items-center gap-2 flex-wrap">
				...
				{/* Polling error */}
				{pollingError && (
					<span className="text-[11px] text-rose-600 border border-rose-200 bg-rose-50 px-3 py-1.5 rounded-sm">
						{pollingError}
					</span>
				)}
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div className="rounded-sm border border-border bg-white p-5">
						...
						<MetaRow
							icon={<Calendar size={13} />}
							label="Closes"
							value={format(new Date(assessment.endsAt), "MMM d, yyyy · HH:mm")}
						/>
						<MetaRow
							icon={<Clock size={13} />}
							label="Duration"
						...
						<MetaRow
							...
							value={
								assessment.isLocationBound
									? assessment.location || "Location-Bound"
									: "Anywhere"
							}
						/>
						<MetaRow
							icon={<Lock size={13} />}
							label="Password"
							value={assessment.passwordProtected ? "Protected" : "None"}
						/>
						<MetaRow
					...
					<div className="rounded-sm border border-border bg-white p-5 md:col-span-2">
						<SectionLabel>
							<span className="flex items-center gap-1.5">
								<Users size={11} /> Assigned Classes
							</span>
						</SectionLabel>
						{assessment.classes.length === 0 ? (
							...
							<div className="flex flex-wrap gap-2">
								{assessment.classes.map((c: any) => (
									<span
										key={c.id}
										className="inline-flex items-center px-3 py-1.5 rounded-sm border border-border bg-[#f3f2f1] text-[12px] text-[#1e293b]"
									>
										{c.className}
									</span>
								))}
					...
					<div className="rounded-sm border border-border bg-white overflow-hidden">
						...
						<table className="w-full text-sm">
							<thead className="bg-[#f3f2f1] border-b border-border">
								<tr>
									<th className="px-5 py-2.5 text-left text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.05em] w-10">
										#
									</th>
									<th className="px-5 py-2.5 text-left text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.05em]">
										Section
									</th>
									<th className="px-5 py-2.5 text-left text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.05em]">
										Type
									...
									<th className="px-5 py-2.5 text-right text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.05em]">
										Marks
									</th>
								</tr>
							</thead>
								...
								{assessment.sections.map((section, secIdx) => {
									...
									const pct =
										assessment.totalMarks > 0
											? Math.round((sectionMarks / assessment.totalMarks) * 100)
											: 0;
									return (
										<tr
											key={section.id}
										...
										>
											<td className="px-5 py-3.5">
												<div className="flex h-5 w-5 items-center justify-center rounded-sm bg-primary text-white text-[10px] font-semibold">
													{secIdx + 1}
												</div>
											</td>
											...
											<td className="px-5 py-3.5 text-[11px] text-muted-foreground">
												...
												section.requiredQuestionsCount < units
							...
							<tfoot className="border-t border-border bg-[#f3f2f1]">
								<tr>
									...
									>
										...
										across {assessment.sections.length} section
		...
		if (!resultsData)
			return (
				<div className="rounded-sm border border-border bg-[#f3f2f1] p-12 text-center">
					<p className="text-[13px] text-muted-foreground">
						Results are not available for draft assessments.
					</p>
				</div>
			);
			...
			<div className="space-y-4">
				...
				{gradingStatus === "NOT_GRADED" &&
					...
					resultsData.status === "PUBLISHED" && (
						<div className="flex items-center gap-2 px-4 py-3 rounded-sm border border-amber-200 bg-amber-50 text-[12px] font-semibold text-amber-700">
							<AlertTriangle size={13} /> Close the assessment first to enable
							grading.
						</div>
					)}
				...
				<div className="grid grid-cols-1 md:grid-cols-[1fr_200px] gap-4">
					...
					<div className="rounded-sm border border-border bg-white p-5 flex flex-col items-center justify-center gap-3">
						<SectionLabel>
							<span className="self-start w-full block">Submission Rate</span>
						</SectionLabel>
						<div className="relative h-28 w-28">
							<Doughnut data={doughnutData} options={doughnutOptions} />
							<div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
								<span className="text-[22px] font-semibold text-[#1e293b]">
									{submissionRate}%
								</span>
				...
				<div className="rounded-sm border border-border bg-white overflow-hidden">
					<div className="flex items-center justify-between px-5 py-3 border-b border-border gap-3">
						<SectionLabel>
							<span className="mb-0">Student Results</span>
						</SectionLabel>
						<div className="flex items-center gap-3 ml-auto">
							...
							<span className="text-[11px] text-muted-foreground shrink-0">
								{tableRows.length} students
							</span>
						</div>
					</div>
					<div className="flex flex-wrap items-center gap-2 px-5 py-3 border-b border-border bg-slate-50/50">
						{resultFilterOptions.map((filter) => {
							const active = resultsFilter === filter.key;
							return (
								<button
									key={filter.key}
									type="button"
									...
									className={`inline-flex items-center gap-1.5 rounded-sm border px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.05em] transition-colors ${
										active
											...
											: "border-border bg-white text-muted-foreground hover:text-[#1e293b]"
									}`}
								>
									{filter.label}
					...
					{resultsData.enrolledStudents.length === 0 ? (
						<div className="px-5 py-12 text-center">
							...
							/>
							<p className="text-[13px] text-muted-foreground">
								No students enrolled in the assigned classes.
							</p>
						</div>
					) : (
						<table className="w-full text-sm">
													...
													{sub?.score != null ? (
														(() => {
															const pct = Math.round(
																(sub.score / resultsData.totalMarks) * 100,
															);
															const color =
																pct >= 70
																	? "#22c55e"
			...
			<div className="px-4 py-5 md:px-6 lg:px-8 max-w-[1280px] pb-16 space-y-5">
				...
				<div className="rounded-sm border border-border bg-white overflow-hidden shadow-[0_1px_3px_0_rgba(0,0,0,0.04)]">
					...
					<div className="px-5 py-3.5 flex flex-wrap items-center gap-2.5 border-b border-[#f1f5f9] bg-[#fafaf9]">
						<span
							className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-sm text-[11px] font-bold border tracking-[0.04em] ${typeBadge[assessment.type]}`}
						>
							<BookOpen size={10} />
							{assessment.type}
						</span>
						...
						<span
							className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-sm text-[11px] font-semibold border ${status.cls}`}
						>
					...
					<div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-[#f1f5f9]">
						...
						/>
						<StatTile
							value={assessment.sections.length}
							label="Sections"
