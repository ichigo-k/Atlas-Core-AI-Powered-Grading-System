  ...
  return (
    <div className="space-y-3">
      ...
      {q.answer?.fileUrl ? (
        ...
        <div className="rounded-sm border border-border bg-[#f3f2f1] px-4 py-3 text-[13px] text-[#1e293b] leading-relaxed whitespace-pre-wrap">
          {q.answer.answerText}
        </div>
      ) : (
...
function AttemptSwitcher({
  ...
  const idx = attempts.findIndex((a) => a.attemptId === current)
  ...
  return (
    <div className="flex items-center gap-2 rounded-sm border border-border bg-white px-4 py-2.5">
      ...
      {attempts.map((a) => {
        ...
        return (
          <button
            ...
            className={`inline-flex items-center gap-1.5 h-7 px-2.5 rounded-sm text-[11px] font-semibold transition-colors ${
              isCurrent
      ...
      <div className="ml-auto flex gap-1">
        <button
          onClick={() => idx > 0 && router.push(`/lecturer/assessments/${assessmentId}/results/attempts/${attempts[idx - 1].attemptId}`)}
          disabled={idx <= 0}
          className="h-6 w-6 flex items-center justify-center rounded-sm border border-border text-muted-foreground hover:bg-[#f3f2f1] disabled:opacity-40 transition-colors"
        >
          <ChevronLeft size={12} />
        </button>
        <button
          onClick={() => idx < attempts.length - 1 && router.push(`/lecturer/assessments/${assessmentId}/results/attempts/${attempts[idx + 1].attemptId}`)}
...
export function AttemptDetailView({
  ...
  // Count questions that have a lecturer override in this attempt
  ...
  const sections: { name: string; questions: QuestionDetail[] }[] = []
  ...
  for (const q of detail.questions) {
    if (!seen.has(q.sectionName)) {
      ...
      sections.push({ name: q.sectionName, questions: arr })
    }
  ...
  return (
    <div className="px-4 py-5 md:px-6 lg:px-8 max-w-[900px] space-y-5 pb-16">
      ...
      <div className="rounded-sm border border-border bg-white overflow-hidden shadow-[0_1px_3px_0_rgba(0,0,0,0.04)]">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {detail.plagiarismFlagged && (
              <span className="inline-flex items-center gap-1.5 rounded-sm border border-[#fecaca] bg-[#fee2e2] px-2.5 py-1 text-[11px] font-semibold text-[#991b1b]">
                <ShieldAlert size={12} /> Plagiarism Flagged
              </span>
            )}
        ...
        {/* Score bar */}
        <div className="px-5 pb-4 space-y-2 border-t border-[#f1f5f9] pt-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-[12px] font-semibold text-muted-foreground">Score</span>
              {overrideCount > 0 && (
                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-sm bg-[#dce6f7] border border-[#b8cef0] text-[10px] font-semibold text-primary">
                  {overrideCount} override{overrideCount !== 1 ? "s" : ""}
                </span>
              )}
            </div>
            <span className="text-[16px] font-bold text-[#1e293b] tabular-nums">{scoreDisplay}</span>
          </div>
          {detail.score !== null && <ScoreBar score={detail.score} max={detail.totalMarks} />}
      ...
      {sections.length > 0 && (
        <div className="rounded-sm border border-border bg-white overflow-hidden">
          ...
          <div className="flex items-center border-b border-border overflow-x-auto">
            {sections.map((s, i) => {
              const isActive = s.name === activeSection
              const sectionMarks = s.questions.reduce((acc, q) => acc + q.marks, 0)
              const type = s.questions[0]?.sectionType
              return (
                <button
                  key={s.name}
                  onClick={() => setActiveSection(s.name)}
                  className={`flex items-center gap-2 px-5 py-3 text-[13px] font-semibold border-b-2 whitespace-nowrap transition-colors shrink-0 ${
                    isActive
                      ? "border-primary text-primary bg-[#f5f8ff]"
                      : "border-transparent text-muted-foreground hover:text-[#1e293b] hover:bg-[#f3f2f1]"
                  }`}
                >
                  <span className={`flex h-5 w-5 items-center justify-center rounded-sm text-[10px] font-bold ${
                    isActive ? "bg-primary text-white" : "bg-[#e2e8f0] text-[#475569]"
                  }`}>
                    {i + 1}
                  </span>
                  {s.name}
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-sm font-semibold ${
                    isActive ? "bg-[#dce6f7] text-primary" : "bg-[#f3f2f1] text-muted-foreground"
                  }`}>
                    {s.questions.length}Q · {sectionMarks}m
                  </span>
                  {type && (
                    <span className={`text-[9px] font-bold uppercase tracking-[0.04em] px-1.5 py-0.5 rounded-sm ${
                      type === "OBJECTIVE"
                        ? "bg-[#dbeafe] text-[#1e40af]"
                        : "bg-[#f3e8ff] text-[#6b21a8]"
                    }`}>
                      {type === "OBJECTIVE" ? "MCQ" : "SUB"}
                    </span>
                  )}
                </button>
              )
          ...
          {currentSection && (
            <div className="divide-y divide-[#f1f5f9]">
              {currentSection.questions.map((q, idx) => (
                <div key={q.id}>
                  {/* Question header */}
                  <div className="flex items-start justify-between gap-3 px-5 py-4 bg-[#fafaf9] border-b border-[#f1f5f9]">
                    <div className="flex items-start gap-2.5 flex-1 min-w-0">
                      <span className="shrink-0 flex h-5 w-5 items-center justify-center rounded-sm bg-primary text-white text-[10px] font-bold mt-0.5">
                        {idx + 1}
                      </span>
                      <p className="text-[13px] font-medium text-[#1e293b] leading-relaxed">{q.body}</p>
                    </div>
                    <span className="shrink-0 text-[11px] font-semibold text-muted-foreground tabular-nums whitespace-nowrap">
                  ...
                  <div className="px-5 py-4">
                    {q.sectionType === "OBJECTIVE" ? (
                      <McqQuestion q={q} assessmentId={assessmentId} attemptId={detail.attemptId} />
                    ) : (
