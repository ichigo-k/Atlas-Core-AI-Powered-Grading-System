...
const statusBadge: Record<string, { cls: string; dot: string }> = {
	...
	CLOSED: {
		cls: "bg-slate-200 text-slate-500 border-slate-300",
		dot: "bg-slate-400",
	},
};
...
function Chip({
	...
	return (
		<span
			className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium border ${className}`}
		>
			{children}
		</span>
	);
...
export default function AssessmentResultsView({
		...
		async function poll() {
			...
			try {
				...
				if (consecutiveFailures >= 3) {
					if (intervalId) clearInterval(intervalId);
					setPollingError(
						"Unable to check grading status. Please refresh the page manually.",
					);
				}
			}
		}
	...
	const gradedCount = data.submissions.filter(
	...
	const submissionByStudent = new Map(
		data.submissions.map((s: any) => [s.studentId, s]),
	);

	// Score distribution buckets for bar chart
	const scoredSubmissions = data.submissions.filter(
		(s: any) => s.score != null,
	);
	...
	const rows = useMemo(() => {
		...
		const filtered = data.enrolledStudents.filter((s) => {
		...
		return [...filtered].sort((a, b) => {
			...
			if (sortKey === "name") cmp = a.name.localeCompare(b.name);
				...
				cmp =
					(statusOrder[subA?.status as keyof typeof statusOrder] ?? 2) -
	...
	const SortBtn = ({ col, label }: { col: SortKey; label: string }) => (
		...
		>
			...
			<ArrowUpDown
				size={11}
				className={sortKey === col ? "text-[#002388]" : "text-slate-300"}
			/>
		</button>
	);
	...
	function handleStartGrading() {
		startGrading(async () => {
			const res = await fetch(
				`/api/lecturer/assessments/${data.id}/start-grading`,
				{
					method: "POST",
				},
			);
	...
	function handleReleaseResults() {
		startReleasing(async () => {
			const res = await fetch(
				`/api/lecturer/assessments/${data.id}/release-results`,
				{
					method: "POST",
				},
			);
	...
	function handleUnreleaseResults() {
		startUnreleasing(async () => {
			const res = await fetch(
				`/api/lecturer/assessments/${data.id}/unrelease-results`,
				{
					method: "POST",
				},
			);
	...
	async function handleRegrade(attemptId: number) {
		...
		try {
			...
			if (res.status === 429) {
				...
				toast.error(
					`Re-grading is rate limited. Try again in ${mins} minute${mins !== 1 ? "s" : ""}.`,
				);
				return;
			}
			if (!res.ok) {
		...
		<div className="px-4 py-5 md:px-6 lg:px-8 max-w-[1280px] space-y-5 pb-12">
			...
			<div className="rounded-sm border border-slate-200 bg-white overflow-hidden">
				<div className="p-6 space-y-2">
					<div className="flex flex-wrap items-center gap-2">
						...
						<span
							className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-medium border ${status.cls}`}
						>
					...
					<p className="text-[11px] text-muted-foreground">
						{format(new Date(data.startsAt), "MMM d, yyyy HH:mm")} —{" "}
						{format(new Date(data.endsAt), "MMM d, yyyy HH:mm")}
					</p>
				</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-[1fr_220px] gap-4">
				...
				<div className="rounded-sm border border-slate-200 bg-white p-5 flex flex-col items-center justify-center gap-4">
					<p className="text-[11px] font-semibold text-slate-400 uppercase tracking-[0.12em] self-start">
						Submissions
					</p>
					...
					<div className="flex items-center gap-4 text-xs text-slate-500">
						<span className="flex items-center gap-1.5">
							<span className="h-2.5 w-2.5 rounded-sm bg-[#002388]" />
			...
			<div className="rounded-sm border border-slate-200 bg-white overflow-hidden">
				<div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 gap-3">
					<p className="text-[11px] font-semibold text-slate-400 uppercase tracking-[0.12em] shrink-0">
						Student Results
					</p>
					<input
						value={search}
									...
									return (
										<tr
											...
											onClick={() => {
												if (!sub) return;
												router.push(
													`/lecturer/assessments/${data.id}/results/attempts/${sub.attemptId}`,
												);
											}}
										>
													...
													<div className="flex flex-col items-end gap-1">
														{(() => {
															...
															const pct = Math.round(
																(displayScore / data.totalMarks) * 100,
															);
															const color =
																pct >= 70
																	? "#22c55e"
