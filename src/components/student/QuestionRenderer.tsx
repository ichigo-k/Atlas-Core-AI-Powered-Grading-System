...
import CodeInput, { type SupportedLang } from "./CodeInput";
...
export default function QuestionRenderer({
	...
	const [selectedOption, setSelectedOption] = useState<number | null>(
		question.existingAnswer?.selectedOption ?? null,
	);
	const [fillText, setFillText] = useState(
		question.existingAnswer?.answerText ?? "",
	);
	...
	async function persistAnswer(payload: {
		...
		setSaveState("saving");
		const result = await saveAnswer(attemptId, question.id, payload);
		if ("error" in result) {
			setSaveState("error");
			return;
		}
		setSaveState("saved");
		if (savedResetRef.current) clearTimeout(savedResetRef.current);
		savedResetRef.current = setTimeout(() => setSaveState("idle"), 2500);
	}

	function scheduleDebounce(payload: {
		answerText: string | null;
		selectedOption: number | null;
		fileUrl: string | null;
	}) {
		...
		debounceRef.current = setTimeout(() => void persistAnswer(payload), 900);
	}
	...
	const displayOptions = shuffledOptions
		? shuffledOptions.map((i: number) => rawOptions[i] ?? "")
		: rawOptions;
		...
		if (locked) {
			return (
				<div className="rounded-lg border border-dashed border-[#e5e7eb] bg-[#fafafa] px-5 py-6 text-center">
					...
					>
						You have already answered the required number of questions in this
						section.
					</p>
				</div>
			);
			...
			return (
				<fieldset className="flex flex-col gap-0.5">
					...
					{displayOptions.map((opt, displayIdx) => {
						const originalIdx = shuffledOptions
							? shuffledOptions[displayIdx]
							: displayIdx;
					...
					{displayOptions.length === 0 && (
						<p
							className="text-sm text-[#9ca3af] italic py-2"
							style={{ fontFamily: FONT_SANS }}
						>
			...
			if (assessmentType !== "ASSIGNMENT") {
				return (
					<div className="flex items-start gap-3 rounded-xl border border-[#fde68a] bg-[#fffbeb] px-5 py-4">
						...
						<p
							className="text-[13px] text-[#92400e] leading-relaxed"
							style={{ fontFamily: FONT_SANS }}
						>
							File upload is only available for <strong>Assignment</strong> type
							assessments. This question cannot be answered here.
						</p>
					</div>
				);
			}
				...
				<PdfDropZone
					onFile={async (file) => {
						...
						try {
							...
							onAnswerChange?.(question.id, payload);
							if (!simulation) await persistAnswer(payload);
						} catch (err: unknown) {
							const message =
						...
						<button
							...
							onClick={() => {
								if (lastPayloadRef.current)
									void persistAnswer(lastPayloadRef.current);
							}}
							className="flex items-center gap-1.5 text-[11px] font-semibold text-red-600 hover:underline"
						>
