...
export async function createOrResumeAttempt(
	...
	try {
		...
		if (assessment.passwordProtected) {
			if (!password || password !== assessment.accessPassword)
		...
		const totalCount = await prisma.assessmentAttempt.count({
		...
		if (assessment.shuffleQuestions) {
			...
			questionOrder = shuffleWithSeed(questions.map((q: any) => q.id)).map(
				(id: any) => ({ questionId: id }),
			);
		}
...
export async function submitAttemptInternal(
	...
	try {
		...
		if (!currentAttempt) {
			console.error("[submitAttemptInternal] Attempt not found", {
				attemptId,
				assessmentId,
			});
			return { error: "NOT_FOUND" };
		...
		if (Array.isArray(activeIds) && activeIds.length > 0) {
			...
			const droppedIds = answers
				.filter((a: any) => !activeSet.has(a.questionId))
				.map((a: any) => a.questionId);
			if (droppedIds.length > 0) {
				await prisma.studentAnswer.deleteMany({
					where: { attemptId, questionId: { in: droppedIds } },
				});
			}
		...
		for (const answer of scoredAnswers) {
			...
			if (
				optCount !== null &&
				(question.correctOption! < 0 || question.correctOption! >= optCount)
			) {
		...
		const existingLog = Array.isArray(currentAttempt.tabSwitchLog)
			? currentAttempt.tabSwitchLog
			: [];
