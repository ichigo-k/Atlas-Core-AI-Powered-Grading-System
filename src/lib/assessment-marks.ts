import type { SectionFormState } from "./assessment-types"

// A "unit" is one standalone question or one whole group. Marks for a group
// unit are its declared total marks.
export function sectionUnitMarksList(sec: SectionFormState): number[] {
  return [
...
export function sectionUnitMarks(sec: SectionFormState): number {
...
export function sectionUnitCount(sec: SectionFormState): number {
